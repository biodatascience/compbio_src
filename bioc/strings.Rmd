...
*GRanges*. Here we choose the `NCBI` version of the genome. 
Note that the difference between the "UCSC" and "NCBI/Ensembl" style
of chromosomes is `chr1` vs `1`.
...
we know the cDNA file exists for the same genome and gene
version. Another way, if we only have a *GRangesList* for the
transcripts (not genes) is to use the `extractTranscriptSeqs` function
from the GenomicFeatures package. This function takes two arguments:
a *BSgenome* object and the *GRangesList* of the exons-by-transcript.
...
txp.seq2 <- extractTranscriptSeqs(Hsapiens, txp)
txp.seq2
txp.seq == txp.seq2
```
