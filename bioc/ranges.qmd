...
```{r message=FALSE}
chr1query <- plyranges::read_bed("chr1_query.bed")
txp <- plyranges::read_bed("chr1_100Mb_v86_txps.bed")
...
```{r}
txp[ overlapsAny(txp, chr1query) ]
txp[ overlapsAny(txp, chr1query[2:3]) ]
txp[ overlapsAny(txp, chr1query, ignore.strand=TRUE) ]
```
...
(while the binary operator however doesn't allow for arguments):
