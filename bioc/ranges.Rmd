...
Finally, I want to introduce some functionality we will use a couple
times later in the course: finding overlaps of sets of genomic ranges.
One scenario is that we have two sets and we want to know which ranges
from the two sets overlap each other. We might make a *query* set and
ask which genes are overlapping the set in our query.
