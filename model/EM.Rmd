...
$$ \ell(\theta) = \sum_i \sum_c \log \pi_c f(x_i; \mu_c, \sigma^2_c)^{\mathbb{1}(z_i = c)} $$

However, we don't observe the $z_i$ and so if we attempted maximum 
likelihood estimation, we would be stuck with the probability given 
the parameters $\theta$, which has a sum over components *c* preventing 
