...
represented by *Z*, the (other) parameters as $\theta$, and the data
itself as *X*. Typically, the *Z* takes discrete values (e.g. it may
...
*Expectation maximization* (EM) in an algorithm which helps to tackle such
problems, and was formulated in 
[1977](https://www.jstor.org/stable/2984875). The authors noted that
others had proposed the EM algorithm previously in various domains.
They were however the first to unify the problem and algorithm, and to
provide statistical motivation for the method. Note that the authors
of the 1977 work do not use *Z*, $\theta$, and *X* notation, as above.

# Motivation for EM

We would like to maximize the log likelihood with respect to $\theta$,
but we typically get stuck in that we cannot distribute the log
...
that cannot be separated, because *Z* is not known. If *Z* were known,
...
which was the log likelihood marginalized over assignments *Z*. 
...
equal to it at $\theta$. One can furthermore show that by optimizing this
...
(see [Andrew Ng's course notes](http://cs229.stanford.edu/notes/cs229-notes8.pdf) 
for a proof). The actual algorithm proceeds by first beginning with a
...
Suppose we have observations *X* which arise from two Normal
distributions. 
We will index the distributions by *c*, and the observations by *i*.
The latent variable $z_i$ tells us which distribution a given $x_i$ comes from.
Define $\theta = (\pi_1, \mu_1, \sigma^2_1, \pi_2, \mu_2, \sigma^2_2)$,
where the $\pi_c$ represent the proportion of observations from distribution *c*.
...
*X* and $\theta^t$. For the moment, let's ignore $\sigma^2_c$ and
focus on $\mu_c$.

$$ \textrm{E}_{Z|X,\theta^t} \left[ \log L(\theta; X, Z) \right] $$
...
parameters, say $\mu_1$, and set equal to zero:
