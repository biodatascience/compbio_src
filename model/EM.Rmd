...
problems. The algorithm had been proposed in various domains as early
...
which was the log likelihood marginalized over assignments of *Z*. 
...
[here](https://images.nature.com/full/nature-assets/nbt/journal/v26/n8/extref/nbt1406-S1.pdf), 
...
Suppose we have observations $x_i$ which arise from two Normal distributions.
We will index the distributions by *c*, and the observations by *i*.
The latent variable $z_i$ will be a binomial which tells us which
distribution a given $x_i$ comes from, where 0 corresponds to the
first distribution and 1 corresponds to the second distribution.
If $z_i = 0$, this means that observation *i* comes from the 1st Normal distribution.
Define $\theta = (\pi_1, \mu_1, \sigma^2_1, \pi_2, \mu_2, \sigma^2_2)$,
where the $\pi_c$ represent the proportion of observations from
distribution *c*. $z_i$ then is distributed as a binomial with success
...
parameters, say $\mu_1$, and set it equal to zero:
...
0.07 and 0.14 for the two distributions. We can see we get very
similar parameter estimates. The paper reported the lower CpG
distribution with $\pi = 0.28$, though this depends heavily on the set
