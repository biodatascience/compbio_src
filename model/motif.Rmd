...
Once we've stored these condition probabilities in a matrix, `posterior_z`,
...
```{r eval=FALSE}
1/n * posterior_z[i,p] * (x[i,p + 0:(theta.len-1)] == j)
```
...
for (t in seq(1,niter)) {
  ...
  posterior_z <- matrix(0,nrow=n,ncol=l-theta.len+1)
  for (i in seq_len(n)) {
    for (p in seq_len(l-theta.len+1)) {
      # move a window along row i of x
      snip <- x[i,p + 0:(theta.len-1)]
      # calculate the conditional distribution of Z given X and current theta
      posterior_z[i,p] <- prod(theta[1,snip == 1]) * prod(theta[2,snip == 2]) *
        prod(theta[3,snip == 3]) * prod(theta[4,snip == 4])
    }
  }
  ...
  Sys.sleep(0.1)
  # image(posterior_z)
  # plot(z, apply(posterior_z, 1, which.max), ylab="max density in posterior_z")
  # abline(0,1)
  ...
  for (j in 1:4) {
    for (i in seq_len(n)) {
      for (p in seq_len(l-theta.len+1)) {
        ...
        theta[j,] <- theta[j,] + 1/n * posterior_z[i,p] * (x[i,p + 0:(theta.len-1)] == j)
      }
    }
  }
