...
We parse the condition from the sample attributes and keep only the
naive (untreated) samples:

```{r}
attrs <- colData(rse)$sra.sample_attributes
treatment <- rep("naive", length(attrs))
treatment[grepl("salmonella infection", attrs) & grepl("interferon gamma", attrs)] <- "IFNg_Salmonella"
treatment[grepl("salmonella infection", attrs) & !grepl("interferon gamma", attrs)] <- "Salmonella"
treatment[!grepl("salmonella infection", attrs) & grepl("interferon gamma", attrs)] <- "IFNg"
table(treatment)
rse <- rse[, treatment == "naive"]
dim(rse)
```
...
normalization). We then filter to genes expressed in the majority of
samples, and take a log2 transformation.

```{r}
mat <- transform_counts(rse)
expressed <- rowMeans(mat > 5) > 0.5
mat <- log2(mat[expressed, ] + 1)
dim(mat)
```
...
strong low-dimensional structure, as one might see with distinct
biological conditions or strong batch effects.
...
called `fake` and perform row-wise t-tests across all expressed genes.
I then look at the distribution of p-values across 20 bins defined on
[0,1].
...
y-axis. For the first 500 tests, this looks like:

```{r}
p.sort <- sort(p2)
plot(1:500, p.sort[1:500], ylim = c(0, p.sort[500]), type = "l",
     xlab = "i", ylab = "p")
```
