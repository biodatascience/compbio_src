...
```{r}
gtf <- here("dist","gencode.v25.annotation.gtf.gz")
if (!file.exists(gtf)) {
  download.file("ftp://ftp.ebi.ac.uk/pub/databases/gencode/Gencode_human/release_25/gencode.v25.annotation.gtf.gz", gtf)
}
if (!file.exists(here("dist","gencode.sqlite"))) {
  txdb <- makeTxDbFromGFF(gtf)
  saveDb(txdb, file=here("dist","gencode.sqlite"))
}
```
...
```{r}
txdb <- loadDb(here("dist","gencode.sqlite"))
