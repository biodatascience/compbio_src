...
This experiment consisted of 10 and 11 mice from two different
species. But useful for our demonstration of SVA, it is key that the
...
which mouse comes from which species.
...
levels(coldata$strain) %<>% (function(x) sub("/",".",x))
```

We use DESeq2 to produce a normalized count matrix, which is the input
to SVA. Note that the biological groups (mouse species) were present
