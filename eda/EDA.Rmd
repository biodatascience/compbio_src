...
We use the `stripplot` function, which is in the `lattice` package: 
...
First, I use the `bigpar` command to make the labels really
large, which is advisable for making plots for slides or posters.
Then I plot the number of genes that code for proteins over the total
number of basepairs:
...
Here we could also have used just `%<>%` instead of `%>%` and `->` at
the end.
