...
Note that if you have issues with the following on a Windows machine,
try adding `mode="wb"` to the `download.file()` command.
...
for each sample. It also accounts for sequencing depth (the total
number of reads per sample), which we will discuss in more detail
later in the course. It takes maybe half a minute to do the scaling.
...
We can also change the color palette and map shape to condition as well.
With 13 regions we need to build a palette manually, combining two
RColorBrewer palettes:

```{r}
library(RColorBrewer)
cols13 <- c(brewer.pal(8, "Dark2"), brewer.pal(5, "Set1"))
ggplot(dat, aes(PC1, PC2, col=condition, shape=condition)) +
  geom_point(size=1.5) +
  scale_color_manual(values=cols13) +
  scale_shape_manual(values=0:12)
```

We can look at the distribution of PC1 scores for each region
individually using a faceted density plot:
